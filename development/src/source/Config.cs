...
namespace LambdaFlow
{
    ...
    internal class Config
    {
        ...
        internal static Config CreateConfig(Stream stream)
        {
            Config cfg;

            cfg = Utilities.DecryptJsonFromStream<Config>(stream, integrity: false, config: true) ?? throw new Exception("Embedded config.json malformed.");
            return cfg;
        }
