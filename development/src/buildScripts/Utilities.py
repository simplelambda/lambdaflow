...
def encrypt_file_aes_gcm(key, input_path, output_path = None):
    ...
    with open(normalizePath(output_path), "wb") as f:
        f.write(blob)
...
def remove(target):
    ...
    if not target.exists():
        print(f"File not found: {target.resolve()}")
        return
...
def generate_key_pair(public_key_path, private_key_path):
    public_key_path = normalizePath(public_key_path)
    private_key_path = normalizePath(private_key_path)
...
def sign(files, private_key_path, sign_path):
    private_key_path = normalizePath(private_key_path)
    sign_path = normalizePath(sign_path)
...
def find_first_csproj(start_path = "") -> Path:
    start_path = normalizePath(start_path)
